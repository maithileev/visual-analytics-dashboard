<!-- <script lang="ts">
  ...
  function drawChart(
    ...
    neighborhoodNormalized: RadarData | null,
    ...
    const overallPoints = keys.map((key) => ({
      key,
      value: Math.max(
        safeValue(overallData.normalized[key]),
        MIN_VISUAL_RADIUS,
      ),
    ...
    const selectedPoints =
      ...
      keys.map((key) => ({
        key,
        value: Math.max(
          safeValue(neighborhoodNormalized[key]),
          MIN_VISUAL_RADIUS,
        ),
        rawValue: safeValue(neighborhoodRawMetrics[key]),
      }));
    ...
    keys.forEach((key, i) => {
      ...
      const dist = Math.hypot(
        overallPos[0] - selectedPos[0],
        overallPos[1] - selectedPos[1],
      );
    ...
    g.selectAll(".overall-point")
      ...
      .on("mouseover", (event, d) => {
        ...
        if (selectedPoints) {
          ...
          if (sel)
            selectedVal = `<br/><strong>${formatLabel(sel.key)} (Selected):</strong> ${(sel.rawValue * 100).toFixed(1)}%`;
        }
        tooltip
          .style("visibility", "visible")
          .html(
            `<strong>${formatLabel(d.key)} (Overall):</strong> ${(d.rawValue * 100).toFixed(1)}%` +
              selectedVal,
          )
      ...
      g.selectAll(".selected-point")
        ...
        .on("mouseover", (event, d) => {
          ...
          if (overallPoints) {
            ...
            if (ov)
              overallVal = `<br/><strong>${formatLabel(ov.key)} (Overall):</strong> ${(ov.rawValue * 100).toFixed(1)}%`;
          }
          tooltip
            .style("visibility", "visible")
            .html(
              `<strong>${formatLabel(d.key)} (Selected):</strong> ${(d.rawValue * 100).toFixed(1)}%` +
                overallVal,
            )
            .style("left", event.pageX + 10 + "px")
            .style("top", event.pageY - 28 + "px");
