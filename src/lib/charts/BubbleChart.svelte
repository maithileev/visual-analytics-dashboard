  <!-- <script lang="ts">
    ...
    // Find similar neighborhoods by Euclidean distance (price, rating)
    function findSimilarNeighborhoods(selected, allData, topN = 5) {
      return allData
        .map((d) => ({
          ...d,
          distance: Math.sqrt(
            (d.avg_price - selected.avg_price) ** 2 +
            (d.avg_rating - selected.avg_rating) ** 2
          ),
        }))
    ...
    function showTooltip(event: MouseEvent, d) {
      ...
      tooltip.innerHTML = `
        ...
        Avg. Price: €${d.avg_price.toFixed(2)}<br/>
  ...
  const x = d3
    .scaleLinear()
    .domain(d3.extent(data, (d) => d.avg_price) as [number, number])
    .nice()
    .range([0, chartWidth]);

  const y = d3
    .scaleLinear()
    .domain(d3.extent(data, (d) => d.avg_rating) as [number, number])
    .nice()
    .range([chartHeight, 0]);
    ...
    function findSimilarNeighborhoods(selected, allData, topN = 5) {
      return allData
        .map((d) => ({
          ...d,
          distance: Math.sqrt(
            (d.avg_x - selected.avg_x) ** 2 +
            (d.avg_y - selected.avg_y) ** 2
          ),
        }))
        .filter((d) => d.label !== selected.label)
        .sort((a, b) => a.distance - b.distance)
