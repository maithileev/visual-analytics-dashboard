...
export async function loadAndAggregateTopNeighborhoods(): Promise<NeighborhoodStats[]> {
  const [listingsResp, sentimentResp] = await Promise.all([
    fetch(`${base}/listings-detailed.csv`),
    fetch(`${base}/sentiment-by-neighbourhood.csv`),
    ]);
